...
STOP_TITLES = %w( 書きかけの節のある項目 曖昧さ回避 すべての曖昧さ回避 生年未記載 プロジェクト人物伝項目 未完成の一覧 正確性 解消済み仮リンクを含む記事 編集半保護中の記事 単一の出典 大言壮語的な記述になっている項目 長大な項目名 テンプレート呼び出しで引数が重複しているページ 内容の精度・確度に欠けている可能性のある記事 
人名の曖昧さ回避
良質な記事
参照方法
リンクのみの節がある記事
